...
4. 点击开始转写。
5. 任务完成后下载 TXT、SRT、VTT、TSV、CSV 或 JSON。

### 导出格式

- `TXT`：纯文稿，适合阅读和复制。
- `SRT`：标准字幕格式，时间戳形如 `00:00:01,230 --> 00:00:04,560`。
- `VTT`：WebVTT 标准字幕格式，适合 HTML5 视频和网页播放器。
- `TSV`：分段表格，列为 `start_ms`、`end_ms`、`text`，适合脚本处理。
- `CSV`：分段表格，包含序号、起止时间、起止秒数和文本，适合 Excel。
- `JSON`：完整结构化结果，保留引擎、语言、总时长、segments 和 raw 数据。
